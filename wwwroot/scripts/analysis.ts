...
async function readFile() {
    ...
    let result = await fetch("/analysis/file?filename=" + filename);
    if (result.ok) {
        ...
        let powerSeries: any[] = [];
        let amperageSeries: any[] = [];
        let voltageSeries: any[] = [];
        let factorSeries: any[] = [];
        ...
        for (const key of Object.keys(histogram)) {
            if (min > parseInt(key)) min = parseInt(key);
            if (max < parseInt(key)) max = parseInt(key);
        }
...
function setupMainChart(powerSeries: any[], voltageSeries: any[], amperageSeries: any[], factorSeries: any[]) {
    Highcharts.chart("mainChart", <any>{
        chart: {
            zoomType: 'x',
        },
