...
var chartPower: Highcharts.ChartObject;
var chartVoltage: Highcharts.ChartObject;
var chartAmperage: Highcharts.ChartObject;
var chartPowerFactor: Highcharts.ChartObject;


function setupChart(elementId:string, type:string, color: string, initialData: SeriesData[]) {
...
async function setupConnection() {
    let result = await fetch("/realtime/data");
    if (result.ok) {
        let json = await result.json();
        document.getElementById("lblStatus").textContent = json.state;

        (<HTMLInputElement>document.getElementById("hostname")).value = json.hostname;
        (<HTMLInputElement>document.getElementById("filename")).value = json.filename;
        ...
        if (powerSeries.length > MAX_POINTS) powerSeries.splice(0, powerSeries.length - MAX_POINTS);
        ...
        if (amperageSeries.length > MAX_POINTS) amperageSeries.splice(0, amperageSeries.length - MAX_POINTS);
        ...
        if (voltageSeries.length > MAX_POINTS) voltageSeries.splice(0, voltageSeries.length - MAX_POINTS);
        ...
        if (powerFactorSeries.length > MAX_POINTS) powerFactorSeries.splice(0, powerFactorSeries.length - MAX_POINTS);
...
function setupWebSocket() {
    ...
    ws.onmessage = (ev) => {
        ...
        if (obj.type == "poll") {
            ...
            chartPower.series[0].addPoint(<any>[
            ...
            chartAmperage.series[0].addPoint(<any>[
                new Date(pollStatus.StatusSNS.Time).getTime(),
            ...
            chartVoltage.series[0].addPoint(<any>[
                new Date(pollStatus.StatusSNS.Time).getTime(),
            ...
            chartPowerFactor.series[0].addPoint(<any>[
                new Date(pollStatus.StatusSNS.Time).getTime(),
...
document.getElementById("btnSetup").onclick = function (ev) {
    fetch("/realtime/setup", {
        ...
        body: JSON.stringify({
            hostname: (<HTMLInputElement>document.getElementById("hostname")).value,
            filename: (<HTMLInputElement>document.getElementById("filename")).value
        }),
        method: "POST",
    }).then(async (resp) => {
